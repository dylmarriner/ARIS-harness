...
The key rule is simple: **models propose; the runtime owns authority.** A model, external agent or remote node can provide stronger reasoning, specialist knowledge, coding ability or compute, but it does not become ARIS's controlling identity and cannot bypass the deterministic execution chain.

The ARIS-owned runtime foundation lives in [`packages/aris/runtime`](packages/aris/runtime/README.md). The canonical migration and implementation sequence is [`docs/ARIS_HARNESS_IMPLEMENTATION_PLAN.md`](docs/ARIS_HARNESS_IMPLEMENTATION_PLAN.md).
...
Start with the [development guide](docs/development.md), [architecture documentation](docs/architecture.md), and the [ARIS Harness implementation plan](docs/ARIS_HARNESS_IMPLEMENTATION_PLAN.md).
